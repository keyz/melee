...
static inline void queueClearCommand(CardState* state, const u8* dst, s32 size,
                                     int* result)
{
    CardCmd command;
    ...
    command.x1C = 0;
    command.x8 = 0;
    *result = fn_803AC168((s32*) &command);
}

static inline int queueClearDataBlock(CardState* state, const u8* dst,
                                      s32 size)
{
    int result;
    ...
    queueClearCommand(state, dst, size, &result);
    return result;
}
...
static inline void queueDataBlockFinal(CardState* state, s32 block_idx,
                                       u8* dst, s32 size, int* result)
{
    u32 sector_size = state->x8;
    u32 temp = state->x24 + sector_size;
    u32 idx;
    ...
    idx = block_idx + idx;
    offset = sector_size * idx;
    *result = queueCardCommand2(state, block_idx, dst, size, offset);
}
